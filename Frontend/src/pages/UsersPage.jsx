import { KeyRound, Plus, Search, ShieldCheck, Trash2, UserCog, Users, X } from 'lucide-react'
  ...
  return (
    <AppShell>
      <div className="space-y-6">
        ...
        <section className="grid gap-6 2xl:grid-cols-[0.82fr_1.58fr]">
          <section className="panel p-5">
            <div>
              <p className="text-sm text-slate-400">Selected user</p>
              <h3 className="mt-2 text-xl font-semibold text-white">
                {editForm.id ? 'Update profile, role, and credentials' : 'Choose a user from the directory'}
              </h3>
            </div>

            {!editForm.id ? (
              <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-5 text-sm leading-7 text-slate-400">
                Pick a user from the table to update their name, email, role, profile description, or password.
              </div>
            ) : (
                ...
                <form onSubmit={handleEditSubmit} className="space-y-4">
                  ...
                  <label className="block">
                    ...
                    <textarea
                      ...
                      className="theme-input min-h-[112px] w-full rounded-2xl px-4 py-3 text-sm outline-none"
                  ...
                  <button
                    ...
                    className="w-full rounded-2xl border border-emerald-400/20 bg-emerald-400/10 px-5 py-3 text-sm font-semibold text-emerald-100 transition hover:bg-emerald-400/15 disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {isEditSubmitting ? 'Saving changes...' : 'Save changes'}
                  </button>
                </form>

                <form onSubmit={handlePasswordSubmit} className="space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4">
